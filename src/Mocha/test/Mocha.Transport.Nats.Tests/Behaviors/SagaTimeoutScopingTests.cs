...
public class SagaTimeoutScopingTests(JetStreamFixture fixture)
{
    ...
    {
        // arrange
        // The framework names its saga timeout endpoint after SagaTimedOutEvent, and the naming
        // conventions only host-scope Subscribe routes, so every saga-hosting service derives the same
        // bare 'saga-timed-out' endpoint. That means one durable shared between services and one set
        // of fault subjects fought over. Declaring the endpoint under that same derived name folds
        // into it, so the durable and both fault subjects can be scoped per service.
