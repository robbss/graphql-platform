...
public class PolymorphicFunnelTests(JetStreamFixture fixture)
{
    ...
    {
        // arrange
        // A handler bound to an interface gets the interface's own subject, which nothing publishes
        // to: a publish resolves its subject from the concrete runtime type. Naming the concrete
        // subjects puts the whole family on one durable, which is what keeps them ordered relative to
        // each other. The interface subject stays in the filter list harmlessly.
        ...
        builder.Services
            ...
            .AddNats(nats =>
            {
                nats.ServiceName("polymorphic-funnel");
                ...
                nats.DeclareStream("POLYMORPHIC_FUNNEL")
                    ...
                    .Subject("locker-commands_error")
                    .Subject("locker-commands_skipped")
