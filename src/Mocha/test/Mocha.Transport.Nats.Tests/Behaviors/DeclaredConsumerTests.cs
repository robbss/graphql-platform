...
public class DeclaredConsumerTests(JetStreamFixture fixture)
{
    ...
    {
        // arrange
        // DeclareConsumer is the only way to reach AckWait and AckProgressEvery, so it is the
        // documented path for a long-running handler. Naming the durable an endpoint already derives
        // must not discard the subjects that endpoint contributes, which previously left the consumer
        // with no subjects and failed start-up.
        ...
        builder.Services
            ...
            .AddNats(nats =>
            {
                ...
                // entry assembly name.
                nats.ServiceName("declared-consumer")
