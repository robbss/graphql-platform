...
public class ServiceShapeTests(JetStreamFixture fixture)
{
    ...
    {
        // arrange
        // The realistic service shape: several handlers, one of them on a named endpoint funnelling a
        // command family, and a stream declared under the name the service name derives. The
        // convention stream for the remaining handlers resolves to that same name, so its subjects
        // have to be folded in. Dropping them leaves those handlers' subjects uncaptured, which shows
        // up either as a start-up failure naming an unrelated subject or as a publish that times out.
        ...
        builder.Services
            ...
            .AddNats(nats =>
            {
                nats.ServiceName("shape-service").AutoProvision(true);
                ...
                nats.DeclareStream("SHAPE_SERVICE")
                    ...
                    .Subject("shape-commands_error")
                    .Subject("shape-commands_skipped")
                    .AutoProvision(true);
            });
