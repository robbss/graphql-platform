...
public class DeclaredStreamConflictTests(JetStreamFixture fixture)
{
    ...
    {
        // arrange
        // A convention stream yields when something already owns its subjects, because it is shared
        // by every service touching them. A declared stream must not: discarding what the caller
        // asked for by name turns a configuration mistake into a publish failure much later, with
        ...
        builder.Services
            ...
            .AddNats(nats =>
            {
                nats.ServiceName("conflict-declarer");
    ...
    public async Task ProvisionAsync_Should_DropRedundantSubjects_When_AWildcardCoversAnExistingOne()
    {
        ...
        builder.Services
            ...
            .AddNats(nats => nats.ServiceName("narrow-service"));
