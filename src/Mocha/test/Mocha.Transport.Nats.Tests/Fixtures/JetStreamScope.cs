...
public sealed class JetStreamScope : IAsyncDisposable
{
    ...
    internal JetStreamScope(INatsJSContext jetStream, string serviceName)
    {
    ...
    /// <summary>
    /// Gets a service name derived from the test, unique across the suite.
    /// </summary>
    public string ServiceName { get; }

    /// <summary>
    /// Derives a service name from the calling test that is stable across runs and short enough to
    /// keep the stream name it produces within the length NATS recommends.
    /// </summary>
    internal static string DeriveServiceName(string testName, string filePath)
    {
        var hash = Convert.ToHexString(
            SHA256.HashData(Encoding.UTF8.GetBytes(filePath + "." + testName)))[..8];

        return "svc-" + hash.ToLowerInvariant();
    }
...
{
    /// <summary>
    /// Wipes the server and returns a scope naming this test's service.
    /// </summary>
        ...
        [CallerFilePath] string filePath = "")
    {
        ...
        return new JetStreamScope(jetStream, JetStreamScope.DeriveServiceName(testName, filePath));
    }
}
