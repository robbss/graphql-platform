...
internal static partial class Logs
{
    ...
    [LoggerMessage(
        LogLevel.Warning,
        "The transport is named '{TransportServiceName}' but the host is named "
        + "'{HostServiceName}'. The transport name only names the convention stream; durable "
        + "consumer names are scoped by the host name, which falls back to the entry assembly name. "
        + "Two services sharing a host name share a durable and compete for messages rather than "
        + "each receiving a copy. Set the host service name to match, through the messaging host "
        + "builder or the SERVICE_NAME environment variable.")]
    public static partial void DivergentServiceNames(
        this ILogger logger,
