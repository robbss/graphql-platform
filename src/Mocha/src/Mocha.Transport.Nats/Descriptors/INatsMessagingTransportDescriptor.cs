    ...
    , IMessagingDescriptor<NatsTransportConfiguration>
{
    ...
    /// <summary>
    /// Sets the service name used to derive the default stream name and its subject prefix.
    /// </summary>
    /// <param name="serviceName">The service name, for example <c>order-service</c>.</param>
    /// <returns>The descriptor for method chaining.</returns>
    INatsMessagingTransportDescriptor ServiceName(string serviceName);
