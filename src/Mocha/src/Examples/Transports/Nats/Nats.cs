...
// Two different service names. This one scopes durable consumer names, and would otherwise fall
// back to the entry assembly name.
bus.ConfigureMessageBus(mocha => mocha.Host(host => host.ServiceName("order-service")));

bus.AddEventHandler<OrderPlacedHandler>()
    .AddNats(nats =>
    {
        // This one names the convention stream: ORDER_SERVICE.
